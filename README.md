...
The first playable handling experiment is live. Press to summon the next tortoise in a random three-dimensional orientation. Drag away from that point to steer its rotation rate, return toward the starting point to steady it, then release to drop. Placement is temporarily locked over the stack so this experiment isolates rotation control. The build uses the Blender character study with a simplified Rapier collider, automatic placement height, platform-contact fall detection, an orthographic tracking camera, score, and restart.
