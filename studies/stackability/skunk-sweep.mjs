...
// Skunk proxy: exposed low back, small head, broad S-shaped tail behind rump.
shapes.skunk=[box([.52,.76,.32],[0,0,0],1,.10),box([.27,.32,.20],[0,-.95,-.10],.5,.07)];
for(const x of [-.38,.38])for(const y of [-.48,.48])shapes.skunk.push(box([.14,.18,.13],[x,y,-.42],1.2,.04));
// Overlapping rounded segments preserve the tail's concave side profile.
for(const [y,z,hy,hz] of [[.91,.04,.23,.20],[1.18,.32,.23,.24],[1.26,.68,.22,.24],[1.23,1.03,.24,.24],[1.40,1.31,.32,.20],[1.64,1.26,.23,.20]])shapes.skunk.push(box([.39,hy,hz],[0,y,z],.40,.07));
...
function fixture(w,n,p,rot,fixed=false,tweak=false){
 ...
 if(tweak&&n==='skunk')ss=ss.map((s,i)=>{if(i>=6){s.p[2]=-.1+(s.p[2]+.1)*.75;s.h[2]*=.75;}return s;});
 if(false&&n==='dragonfly')ss=ss.map(s=>{if(Math.abs(s.p[0])>.5)s.p[2]=.32;return s;});
 if(false&&['ram','zebra'].includes(n))ss=ss.map(s=>{if(s.p[2]<-.5){s.p[0]*=1.3;s.h[0]*=1.2;}return s;});
 if(false&&n==='armadillo'){ss[1].h[0]=.68;ss[1].h[1]=.80;}
...
for(const tweak of [false,true])for(const support of ['base',...species])for(const incoming of species){
 if(incoming!=='skunk'&&support!=='skunk')continue;
 for(const pose of poses)results.push({tweak,support,incoming,pose:pose.label,yaw:pose.yaw,...run(support,incoming,pose,tweak)});
}
fs.writeFileSync(new URL('skunk-results.json',import.meta.url),JSON.stringify({method:'Same original harness; only skunk tail height changes in variant (25% lower); all other proxies unchanged',shapes,results},null,2));
for(const tweak of [false,true])for(const role of ['incoming','support']){
const r=results.filter(r=>r.tweak===tweak&&r[role]==='skunk');
console.log(JSON.stringify({tweak,role,total:r.length,stable:r.filter(r=>r.outcome==='stable').length,unavailable:r.filter(r=>r.outcome.startsWith('support_')).length,moving:r.filter(r=>r.outcome==='moving').length,maxUp:Math.max(...r.map(r=>r.maxUp)),poses:Object.fromEntries([...new Set(r.map(r=>r.pose))].map(p=>[p,r.filter(r=>r.pose===p&&r.outcome==='stable').length+'/'+r.filter(r=>r.pose===p).length]))}));
}
