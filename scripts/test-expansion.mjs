...
const output=new URL('../tmp/expansion-test/',import.meta.url);
...
try{
 await page.goto(`${process.env.TEST_URL ?? "http://127.0.0.1:5173/menagerie/"}?trace&diagnostics&sequence=${job.a},${job.b}&rx=${job.rx}`,{waitUntil:'networkidle'});
