...
const seen=new Set();for(let i=0;i<60&&seen.size<6;i++){seen.add(await page.locator('#game').getAttribute('data-held-species'));await page.locator('#restart').click();}
const results=[];
for(const target of ['armadillo','ram','skunk']){
 ...
 const anomalies=trace.events.filter(e=>e.type==='upward_anomaly');await page.locator(lost?'#drop':'#restart').click();results.push({target,score,lost,anomalies,reset:await page.locator('#score').textContent()});
}
