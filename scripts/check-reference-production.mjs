...
for(const target of ['armadillo','ram','skunk']){
 ...
 const g=await page.locator('#rotation-bubble').evaluate(e=>({x:Number(e.dataset.centerX),y:Number(e.dataset.centerY)}));const x=g.x,y=g.y;await page.mouse.move(x,y);await page.mouse.down();await page.mouse.move(x+45,y+20,{steps:8});await page.waitForTimeout(100);await page.mouse.up();await page.mouse.click(x,y);
