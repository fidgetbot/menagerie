...
The default presentation has no rotation guide, gesture tutorial text, or XYZ axis labeling. With `?bubble=1`, an almost transparent bubble sharing the control's centre and radius fades in only during an active drag; `?bubble=0` and the bare URL hide it. Bubble visibility never changes input mapping. The initial two-line stacking goal fades after the first score. On page load only, the first animal receives a 4.4 rad/s diagonal flick using ordinary damping; reduced-motion preference skips this cue. Ordinary replay and subsequent animals receive no opening flick. Cancellation, blur, and page hiding stop rotation without placing a piece.
