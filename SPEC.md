...
Only the newest released animal can trigger fall tracking, after 350 ms continuously descending unsupported below the stack top. Follow stays locked. Normal tower tracking only rises. Fall/final-view descent is limited to 2 world units/s and 2.5 world units/s² acceleration, slowing near its target; it never rushes to catch a distant body. On loss the final view eases to platform level at fixed zoom/angle, with no forced completion deadline. The Game over overlay and separate Play again button are removed: the bottom Drop button immediately becomes an enabled Play again action, including during the descent. Restart clears camera state and restores Drop. Physics and scoring are unchanged.
